...
import { IP,PORT_9001,API_URL,GET_FLIGHT_DETAILS,GET_FLIGHTS_FOR_ASSET,GOOGLE_API_KEY } from '../Config/siteConfig';
...
export const getAirCraft = (startDate, endDate, airCraftId) => {
  return new Promise((resolve, reject) => {
    ...
    //let apiUrl = `http://103.10.234.158:8081/getFlightsForAsset/${airCraftId}/${startDate} 00|${endDate} 23`;
     let apiUrl = `${IP}${PORT_9001}${GET_FLIGHTS_FOR_ASSET}/${airCraftId}/${startDate} 00|${endDate} 23`;
    axios
      .get(apiUrl)
      .then((results) => {
...
export const getAirCraftDetails = (airCraftId) => {
  ...
  let apiUrl =`${IP}${PORT_9001}${GET_FLIGHT_DETAILS}`;
  return new Promise((resolve, reject) => {
    if (airCraftId) {
